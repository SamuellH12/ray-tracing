...
void update_angle(Camera &cam, sf::RenderWindow &window){
...
vetor get_vel_vector(Camera &cam){
    vetor x = cam.get_v(); x = vetor(x.getX(), 0, x.getZ()).normalized();
    vetor y(0, 1, 0);
    vetor z = cam.get_w(); z = vetor(z.getX(), 0, z.getZ()).normalized();
    double dlt_tm = delta_time();
    ...
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Up))   delta = delta - (x * dlt_tm); else 
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Down)) delta = delta + (x * dlt_tm);

    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Left))  delta = delta - (z * dlt_tm); else 
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Right)) delta = delta + (z * dlt_tm);
    
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Enter))  delta = delta + (y * dlt_tm); else 
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::RShift)) delta = delta - (y * dlt_tm);
...
int main(){
    ...
    auto cena = get_scene_debug(); // (HEIGHT, WIDTH);
    auto &cam = cena.cam;
    ...
    auto Bez = new Bline(std::vector<point>({point(160, 0, 160), point(160, 160, 160), point(160, 160, 0), /*point(0, 0, 0),*/ point(0, 160, 160)}), Color(0.1, 0.9, 0.2), 0.75);
    ...
    while(window.isOpen()){
        ...
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Delete)) break;
        window.clear();
        ...
        if(MODELUZ) update_pos_luz(cena.luzes, cam);
        if(SHOWMETA) update_bezier(Bez->pts, cam);
        ...
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Num8)) SHOWMETA = !SHOWMETA, MODELUZ = false;
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Num7)) MODELUZ = !MODELUZ, SHOWMETA =false;
        Bez->showMeta = SHOWMETA;
        Bez->metat = BEZIERT;
        

        if(modo != 1) update_angle(cam, window);
        ...
        int curr_fps = 1.0/(((double)t-clock()*10.0)/CLOCKS_PER_SEC);
...
// if(modo==1) const double sla = 0.05; // cam.rotatey(sla * velocidade); // cam.set_position( point() + (cam.get_pos() - point()).rotatey(sla * velocidade) );